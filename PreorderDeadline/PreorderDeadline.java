...
public class PreorderDeadline {
    ...
    public static Date getPreorderDeadline () {
        ...
        //System.out.println("date is:" + dateToString(date));
        for (i = 0; i<=47; i++){
            //System.out.println(dateToString(dateArray[i]));
            if ( date.before(dateArray[i]) ) {
                Date out = dateArray[i];
                return out;
            }
        } 
        // if time is 23:59:59:x
        Date tomorrow = stringToDate(fullTomorrow("00:30:00"));
        Date out = tomorrow;
